...
public class ItemDataTypeDao extends AbstractDomainDao<ItemDataType> {
    ...
    public ItemDataType findByItemDataTypeCode(String item_data_type_code) {
        ...
        return (ItemDataType) q.getResultList().stream().findFirst().orElse(null);
    }
    ...
    public ItemDataType findByItemDataTypeId(int item_data_type_id) {
        ...
        ItemDataType result = (ItemDataType) q.getResultList().stream().findFirst().orElse(null);
        return result;
    }
    ...
    public ItemDataType findByItemId(int item_id) {
        ...
        jakarta.persistence.Query q = getEntityManager().createNativeQuery(query, ItemDataType.class);
        ItemDataType result = (ItemDataType) q.getResultList().stream().findFirst().orElse(null);
        return result;
    }
}
