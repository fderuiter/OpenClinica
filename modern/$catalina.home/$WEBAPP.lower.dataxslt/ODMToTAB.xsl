...
<xsl:template priority="1" mode="formRefToDefMapTemplate"
		match="//odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef/odm:FormRef">
		...
		<xsl:if test="@StudyEventOID = $parentEvent/@StudyEventOID">
			<xsl:choose>
				<xsl:when test="@StudyEventRepeatKey">
					...
					<!-- PG - commented to test outside OC 
...
<xsl:if test="$crfSignedExist">
							...
							<xsl:otherwise>
								<xsl:if
									test="preceding-sibling::odm:StudyEventData[1]/@StudyEventRepeatKey != @StudyEventRepeatKey"><xsl:if test="$interviewerNameExist">
										<xsl:text>Interviewer_</xsl:text>
										<xsl:value-of select="$E" />
										<xsl:value-of select="$eventPosition" />
										<xsl:if test="@StudyEventRepeatKey">
...
<xsl:if test="$crfSignedExist">
											<xsl:value-of select="$currentForm/@OpenClinica:Signed"/>
											<xsl:value-of select="$delimiter" />
										</xsl:if>
										<xsl:if test="$crfVersionExist">
...
<xsl:if test="$crfSignedExist">
							...
							<xsl:value-of select="' '"	/>
							<xsl:text>CRF Signature</xsl:text>
								<xsl:choose>
									<xsl:when test="$generateIntHeadersList = 'Yes'"><!-- Use special constants here than '_E' for internal processing -->
										<xsl:value-of select="$itemNameAndEventSep"/>
									</xsl:when>
...
<xsl:template name="studyEventInfoData2">		
		...
		<!--{T<xsl:value-of select="position()"/>:<xsl:value-of select="."/>}-->
			<xsl:if test=". != $tokenizedEventHeaders[last()]"><!--not last	-->
					...
					<!--urrentToken:*<xsl:value-of select="$currentToken"/>*
					...
					<xsl:variable name="numericStart">
						...
						<xsl:if test="ends-with($nextToken,'Event Status')">
							<xsl:value-of select="substring-before($nextToken,concat($delimiter, 'Event Status'))"/>
						</xsl:if>
					...
					<!--colType:<xsl:value-of select="$colType"/>isColForRepeatingEvent:<xsl:value-of select="$isColForRepeatingEvent"/>-->
...
<!--<xsl:text>M</xsl:text>--><xsl:value-of select="$matchSep"/><xsl:value-of select="position()"/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:when>
							<xsl:otherwise>
...
<xsl:when test="@OpenClinica:Signed">
														<td class="table_cell">
															<xsl:value-of select="@OpenClinica:Signed"></xsl:value-of>
														</td>
													</xsl:when>
...
<xsl:when test="@OpenClinica:Signed">
	...
	<xsl:template name="studyCRFAndItemsData">
		...
		<!--tokenizedcrfAndDataItemsHeaders:
			...
			<xsl:variable name="numericStart">
				<xsl:choose>
					...
					<xsl:when test="ends-with($nextToken,'Interview Date')">
						<xsl:value-of select="substring-before($nextToken,concat(concat($delimiter, ' '), 'Interview Date'))"/>
					</xsl:when>
					<xsl:when test="ends-with($nextToken,'CRF Version Status')">
						<xsl:value-of select="substring-before($nextToken,concat(concat($delimiter, ' '), 'CRF Version Status'))"/>
					</xsl:when>
<xsl:when test="ends-with($nextToken,'CRF Signature')">
						<xsl:value-of select="substring-before($nextToken,concat(concat($delimiter, ' '), 'CRF Signature'))"/>
					</xsl:when>
					<xsl:when test="ends-with($nextToken,'Version Name')">
						<xsl:value-of select="substring-before($nextToken,concat(concat($delimiter, ' '), 'Version Name'))"/>
					</xsl:when>
					<xsl:otherwise>
...
<xsl:when test="ends-with($currentToken,'CRF Signature')">
						<xsl:text>CRF Signature</xsl:text>
					</xsl:when>
...
<xsl:if test="$colType = 'CRF Signature'">	
									<xsl:choose> 
										<xsl:when test="$formData/@OpenClinica:Signed">	
											<xsl:value-of select="$formData/@OpenClinica:Signed"></xsl:value-of>
											<xsl:value-of select="$delimiter" />
