...
public class StudyStatusController {
    ...
    public ResponseEntity<Object> updateStatus(
        ...
        // Update study events
        jdbcTemplate.update("UPDATE study_event SET status_id = ? WHERE study_event_definition_id IN (SELECT study_event_definition_id FROM study_event_definition WHERE study_id = ?)", statusId, studyId);
        
        // Update event CRFs
        jdbcTemplate.update("UPDATE event_crf SET status_id = ? WHERE study_event_id IN (SELECT study_event_id FROM study_event WHERE study_event_definition_id IN (SELECT study_event_definition_id FROM study_event_definition WHERE study_id = ?))", statusId, studyId);
        
        // Update item data
        jdbcTemplate.update("UPDATE item_data SET status_id = ? WHERE event_crf_id IN (SELECT event_crf_id FROM event_crf WHERE study_event_id IN (SELECT study_event_id FROM study_event WHERE study_event_definition_id IN (SELECT study_event_definition_id FROM study_event_definition WHERE study_id = ?)))", statusId, studyId);
