...
Calendar.tableMouseOver = function (ev) {
  ...
  if (target == el || target.parentNode == el) {
    ...
    if (
      typeof el.navtype == 'undefined' ||
      (el.navtype != 50 && (el.navtype == 0 || Math.abs(el.navtype) > 2))
    )
      Calendar.removeClass(el, 'active');
...
Calendar.cellClick = function (el, ev) {
  ...
  if (typeof el.navtype == 'undefined') {
    ...
    date = new Date(cal.date);
    if (el.navtype == 0) date.setDateOnly(new Date()); // TODAY
...
Calendar.prototype._init = function (firstDayOfWeek, date) {
  ...
  for (var i = 0; i < 6; ++i, row = row.nextSibling) {
    ...
    for (
      var j = 0;
      j < 7;
      ++j, cell = cell.nextSibling, date.setDate(iday + 1)
    ) {
      iday = date.getDate();
...
Calendar._checkCalendar = function (ev) {
  ...
  var el = Calendar.is_ie
    ? Calendar.getElement(ev)
...
Calendar.prototype.showAtElement = function (el, opts, cpX, cpY) {
  var self = this;
  ...
  function fixPosition(box) {
    ...
    if (Calendar.is_ie && !(Calendar.is_ie7 || Calendar.is_ie8)) {
...
Calendar.prototype._displayWeekdays = function () {
  ...
  for (var i = 0; i < 7; ++i) {
    ...
    if (i) {
      cell.ttip = Calendar._TT['DAY_FIRST'].replace(
        '%s',
  ...
  for (i = 0; i < a.length; ++i) {
    ...
    switch (b[i]) {
      ...
      case '%B':
        for (j = 0; j < 12; ++j) {
          if (
            Calendar._MN[j].substr(0, a[i].length).toLowerCase() ==
            a[i].toLowerCase()
          ) {
            m = j;
            break;
