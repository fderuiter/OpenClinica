  ...
  if (tt_aV[CENTERWINDOW]) {
    ...
    if (tt_aV[STICKY]) {
      if (tt_aV[CENTERALWAYS]) {
        ...
        if (tt_ie) tt_AddEvtFnc(window, 'scroll', Ctrwnd_DoCenter);
        else tt_aElt[0].style.position = 'fixed';
...
ctrwnd.OnKill = function () {
  if (tt_aV[CENTERWINDOW] && tt_aV[CENTERALWAYS]) {
    ...
    if (tt_ie) tt_RemEvtFnc(window, 'scroll', Ctrwnd_DoCenter);
    else tt_aElt[0].style.position = 'absolute';
  }
  return false;
};
// Helper function
