...
window.Calendar = {
    setup: function(config) {
        ...
        var inputElement = typeof config.inputField === 'string' 
            ? (document.getElementById(config.inputField) || document.getElementsByName(config.inputField)[0]) 
            : config.inputField;
        ...
        if (config.button) {
            var btn = typeof config.button === 'string' 
                ? document.getElementById(config.button) 
                : config.button;

            if (btn) {
                var target = (btn.tagName && btn.tagName.toLowerCase() === 'img' && btn.parentNode && btn.parentNode.tagName.toLowerCase() === 'a') 
                    ? btn.parentNode 
                    : btn;
