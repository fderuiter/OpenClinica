...
public class ImportCRFDataService {
    ...
    @Autowired
    public ImportCRFDataService(DataSource ds, Locale locale, CRFDAO _cRFDAO, CRFVersionDAO _cRFVersionDAO, EventCRFDAO _eventCRFDAO, EventDefinitionCRFDAO _eventDefinitionCRFDAO, ItemDAO _itemDAO, ItemDataDAO _itemDataDAO, ItemFormMetadataDAO _itemFormMetadataDAO, ItemGroupDAO _itemGroupDAO, StudyDAO _studyDAO, StudyEventDAO _studyEventDAO, StudyEventDefinitionDAO _studyEventDefinitionDAO, StudySubjectDAO _studySubjectDAO) {
     ...
     */
    public List<EventCRFBean> fetchEventCRFBeans(final ODMContainer odmContainer, final UserAccountBean ub) {
        ...
        return transactionTemplate.execute(new org.springframework.transaction.support.TransactionCallback<List<EventCRFBean>>() {
            @Override
            public List<EventCRFBean> doInTransaction(org.springframework.transaction.TransactionStatus status) {
                try {
                    ...
                    Object _stop_666854 = org.akaza.openclinica.logic.importdata.SubjectDataProcessor.process(subjectDataBeans, new org.akaza.openclinica.logic.importdata.SubjectDataProcessor<Object>() {
                                ...
                                for (CRFVersionBean crfVersionBean : crfVersionBeans) {
                                    ...
                                    if (eventCrfBeans.isEmpty()) {
                                        ...
                                        if ((studyEventBean.getSubjectEventStatus().equals(SubjectEventStatus.SCHEDULED)
                                                ...
                                                .getSubjectEventStatus().equals(SubjectEventStatus.COMPLETED)) && finalUpsert.isNotStarted()) {
                                    ...
                                    for (EventCRFBean ecb : eventCrfBeans) {
                                        ...
                                        if ((finalUpsert.isDataEntryStarted() && ecb.getStage().equals(DataEntryStage.INITIAL_DATA_ENTRY))
                                                || (finalUpsert.isDataEntryComplete() && ecb.getStage().equals(DataEntryStage.DOUBLE_DATA_ENTRY_COMPLETE)))
...
if ("NULL".equals(_stop_666854)) return null;
    ...
    public boolean eventCRFStatusesValid(ODMContainer odmContainer, UserAccountBean ub) {
        ...
        StudyBean studyBean = studyDAO.findByOid(studyOID);
        ArrayList<SubjectDataBean> subjectDataBeans = odmContainer.getCrfDataPostImportContainer().getSubjectData();
        Object _stop_c94890 = org.akaza.openclinica.logic.importdata.SubjectDataProcessor.process(subjectDataBeans, new org.akaza.openclinica.logic.importdata.SubjectDataProcessor<Object>() {
public void process(SubjectDataBean subjectDataBean) {
            ArrayList<StudyEventDataBean> studyEventDataBeans = subjectDataBean.getStudyEventData();
...
if (_stop_c94890 != null) return (Boolean)_stop_c94890;
    ...
    public HashMap<Integer, String> fetchEventCRFStatuses(ODMContainer odmContainer) {
        ...
        ArrayList<SubjectDataBean> subjectDataBeans = odmContainer.getCrfDataPostImportContainer().getSubjectData();
        Object _stop_6614be = org.akaza.openclinica.logic.importdata.SubjectDataProcessor.process(subjectDataBeans, new org.akaza.openclinica.logic.importdata.SubjectDataProcessor<Object>() {
public void process(SubjectDataBean subjectDataBean) {
            ArrayList<StudyEventDataBean> studyEventDataBeans = subjectDataBean.getStudyEventData();
    ...
    public SummaryStatsBean generateSummaryStatsBean(ODMContainer odmContainer, List<DisplayItemBeanWrapper> wrappers, ImportCRFInfoContainer importCrfInfo) {
        ...
        ArrayList<SubjectDataBean> subjectDataBeans = odmContainer.getCrfDataPostImportContainer().getSubjectData();
        countSubjects[0] += subjectDataBeans.size();
        Object _stop_3670cf = org.akaza.openclinica.logic.importdata.SubjectDataProcessor.process(subjectDataBeans, new org.akaza.openclinica.logic.importdata.SubjectDataProcessor<Object>() {
public void process(SubjectDataBean subjectDataBean) {
            ArrayList<StudyEventDataBean> studyEventDataBeans = subjectDataBean.getStudyEventData();
        ...
        ssBean.setEventCrfCount(countEventCRFs[0]);
        ssBean.setStudySubjectCount(countSubjects[0]);
    ...
    public List<DisplayItemBeanWrapper> lookupValidationErrors(Locale locale, ODMContainer odmContainer, UserAccountBean ub,
        ...
        Object _stop_973cd3 = org.akaza.openclinica.logic.importdata.SubjectDataProcessor.process(subjectDataBeans, new org.akaza.openclinica.logic.importdata.SubjectDataProcessor<Object>() {
public void process(SubjectDataBean subjectDataBean) {
            ...
            ArrayList<StudyEventDataBean> studyEventDataBeans = subjectDataBean.getStudyEventData();
            totalEventCRFCount[0] += studyEventDataBeans.size();
            ...
            for (StudyEventDataBean studyEventDataBean : studyEventDataBeans) {
                ...
                int[] ordinal = {1};
                try {
                    ordinal[0] = new Integer(studyEventDataBean.getStudyEventRepeatKey()).intValue();
                } catch (Exception e) {
                    ...
                    if (eventCRFBean != null) {
                        if (permittedEventCRFIds.contains(new Integer(eventCRFBean.getId()))) {
                            ...
                            for (ImportItemGroupDataBean itemGroupDataBean : itemGroupDataBeans) {
                                ...
                                totalItemDataBeanCount[0] += itemDataBeans.size();
                                ...
                                for (ImportItemDataBean importItemDataBean : itemDataBeans) {
                                    ...
                                    ItemFormMetadataDAO itemFormMetadataDAO = this._itemFormMetadataDAO;
                                    ...
                                    if (!itemBeans.isEmpty()) {
                                        ...
                                        if (!metadataBeans.isEmpty()) {
                                            ...
                                            // tbh
                                            try { attachValidator(displayItemBean, importHelper, discValidator[0], hardValidationErrors, locale, eventCRFRepeatKey, studySubjectBean.getOid()); } catch (OpenClinicaException e) { throw new RuntimeException(e); }
                        ...
                        validationErrors[0] = discValidator[0].validate();
                        // totalValidationErrors += validationErrors[0].size();
                        // totalValidationErrors.
                        // totalValidationErrors.addAll(validationErrors);
                        for (Object errorKey : validationErrors[0].keySet()) {
                            // logger.debug(errorKey.toString() + " -- " + validationErrors[0].get(errorKey));
                            // JN: to avoid duplicate errors
                            if (!totalValidationErrors.containsKey(errorKey.toString()))
                                totalValidationErrors.put(errorKey.toString(), validationErrors[0].get(errorKey).toString());
                        ...
                        for (Object errorKey : hardValidator.keySet()) {
                            ...
                            hardValidatorErrorMsgs[0] += hardValidator.get(errorKey) + "<br/><br/>";
                        }
                        ...
                        logger.debug("creation of wrapper: original count of display item beans " + displayItemBeans.size() + ", count of item data beans "
                                + totalItemDataBeanCount[0] + " count of validation errors " + validationErrors[0].size() + " count of study subjects "
                                + subjectDataBeans.size() + " count of event crfs " + totalEventCRFCount[0] + " count of hard error checks "
                                + hardValidator.size());
                        ...
                        // all
                        // the
                        displayItemBeanWrapper[0] = new DisplayItemBeanWrapper(displayItemBeans, true, overwrite, validationErrors[0], studyEventId, crfVersionId,
                        ...
                        validationErrors[0] = new HashMap();
                        discValidator[0] = new DiscrepancyValidator(locale, discNotes);
                ...
                if (displayItemBeanWrapper != null && displayItemBeans.size() > 0)
                    wrappers.add(displayItemBeanWrapper[0]);
            }// after study events
...
// after study subjects
        ...
        if (!hardValidator.isEmpty()) {
            // throw new RuntimeException(new OpenClinicaException(hardValidatorErrorMsgs, ""));
        }
        return wrappers;
    }
    ...
    private void attachValidator(DisplayItemBean displayItemBean, ImportHelper importHelper, DiscrepancyValidator v, HashMap<String, String> hardv,
        ...
        if (rt.equals(org.akaza.openclinica.bean.core.ResponseType.TEXT) || rt.equals(org.akaza.openclinica.bean.core.ResponseType.TEXTAREA)) {
            ...
            v.setAttribute(itemOid, displayItemBean.getData().getValue());
        ...
        // should be filed REF MANTIS 6271
            ...
            String theValue = matchValueWithOptions(displayItemBean, displayItemBean.getData().getValue(), displayItemBean.getMetadata().getResponseSet()
                    .getOptions());
            v.setAttribute(itemOid, theValue);
            ...
            if (theValue == null && displayItemBean.getData().getValue() != null && !displayItemBean.getData().getValue().isEmpty()) {
                ...
                // throw new RuntimeException(new OpenClinicaException("One of your items did not
    ...
    public List<String> validateStudyMetadata(ODMContainer odmContainer, int currentStudyId) {
        ...
        try {
            ...
            if (subjectDataBeans != null) {// need to do this so as not to
                ...
                Object _stop_2087b8 = org.akaza.openclinica.logic.importdata.SubjectDataProcessor.process(subjectDataBeans, new org.akaza.openclinica.logic.importdata.SubjectDataProcessor<Object>() {
public void process(SubjectDataBean subjectDataBean) {
