...
//        throw new InsufficientPermissionException(Page.MENU_SERVLET, resexception.getString("may_not_submit_data"), "1");
    ...
    public void processRequest() throws Exception {
		...
		for(int j = 0; j < studySubjectAudits.size(); j++ ) {
			...
			excelRow = new String[] {
					audit.getAuditEventTypeName(),
					dateTimeFormat(audit.getAuditDate()),
					audit.getUserName(),
					audit.getEntityName(),
					audit.getOldValue(),
					audit.getNewValue()
		...
		for(int j = 0; j < events.size(); j++ ) {
			...
			if(event.getStartTimeFlag()) {
				excelRow = new String[] {
					...
					event.getLocation(),
					dateTimeFormat(event.getDateStarted()),
		...
		for(int eventCount = 0; eventCount < events.size(); eventCount++) {
			...
			for(int j = 0; j < allDeletedEventCRFs.size(); j++ ) {
				...
				excelRow = new String[] {
						deletedEventCRF.getCrfName(),
						deletedEventCRF.getCrfVersion(),
						deletedEventCRF.getDeletedBy(),
			...
			for(int j = 0; j < studyEventAudits.size(); j++ ) {
				...
				excelRow = new String[] {
						...
						studyEvent.getEntityName() + "(" + studyEvent.getOrdinal() +")",
						oldValue,
						newValue						
					};
					for (int i = 0; i < excelRow.length; i++) {
