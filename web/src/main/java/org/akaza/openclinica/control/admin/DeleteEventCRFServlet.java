		...
		if (eventCRFId == 0) {
			...
			if ("confirm".equalsIgnoreCase(action)) {
				...
				for (ItemDataBean itemdata : itemData) {
					...
					ifmdao = org.akaza.openclinica.dao.core.DaoBridge.getDao(ItemFormMetadataDAO.class);
				...
				if(event.getSubjectEventStatus().isCompleted() || event.getSubjectEventStatus().isSigned()){
					...
					event.setUpdater(ub);
                   sedao = org.akaza.openclinica.dao.core.DaoBridge.getDao(StudyEventDAO.class);
                   sedao.update(event);
				}
