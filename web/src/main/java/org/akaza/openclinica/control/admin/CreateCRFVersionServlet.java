...
public class CreateCRFVersionServlet extends SecureController {
    ...
    private boolean canDeleteVersion(int previousVersionId) {
        CRFVersionDAO cdao = org.akaza.openclinica.dao.core.DaoBridge.getDao(CRFVersionDAO.class);
    ...
    private ItemBean isResponseValid(HashMap items, CRFVersionBean version) {
        ItemDAO idao = org.akaza.openclinica.dao.core.DaoBridge.getDao(ItemDAO.class);
        ItemFormMetadataDAO metadao = org.akaza.openclinica.dao.core.DaoBridge.getDao(ItemFormMetadataDAO.class);
        Set names = items.keySet();
        Iterator it = names.iterator();
        while (it.hasNext()) {
            String name = (String) it.next();
