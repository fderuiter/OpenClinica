        ...
        if (note.getEntityId() > 0 && !entityType.equals("")) {
            ...
            if (!StringUtil.isBlank(entityType)) {
                if ("itemData".equalsIgnoreCase(entityType)) {
                    ...
                    StudySubjectBean ssub = (StudySubjectBean) org.akaza.openclinica.dao.core.DaoBridge.getDao(StudySubjectDAO.class).findByPK(ec.getStudySubjectId());
                    note.setStudySub(ssub);
                    StudyEventBean event = (StudyEventBean) org.akaza.openclinica.dao.core.DaoBridge.getDao(StudyEventDAO.class).findByPK(ec.getStudyEventId());
                    note.setEvent(event);
                    if (!StringUtil.isBlank(note.getColumn())) {
