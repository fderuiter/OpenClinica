...
const manifestPath = path.join(__dirname, 'src/main/webapp/dist/.vite/manifest.json');
...
function copyAndReplace(dir) {
    ...
    for (const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
        ...
        } else if (fullPath.endsWith('.jsp') || fullPath.endsWith('.html') || fullPath.endsWith('.htm') || fullPath.endsWith('load_scripts.js')) {
            ...
            if (fullPath.endsWith('load_scripts.js')) {
                ...
                const scriptRegex = /<script[^>]*src="[^"]*includes\/(scriptaculous|effects|ua-parser\.min)\.js(\?load=effects)?"[^>]*><\/script>\s*/g;
                if (scriptRegex.test(content)) {
                    content = content.replace(scriptRegex, '');
                    changed = true;
                }

                const headRegex = /<script[^>]*src="[^"]*\/js\/lib\/head\.min\.js"[^>]*><\/script>\s*/g;
